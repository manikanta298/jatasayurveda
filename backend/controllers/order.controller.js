...
const paymentMethods = asyncHandler(async (req, res) => sendSuccess(res, 200, listEnabledGateways()));

const createOrder = asyncHandler(async (req, res) => {
  const { customerName, customerEmail, customerPhone, shippingAddress, cartItems, couponCode, notes, paymentMethod } = req.body;
  ...
  const { items, subtotalPaise, shippingPaise, discountPaise, totalPaise, appliedCoupon } = await priceOrder({ cartItems, couponCode });
...
const verifyPayment = asyncHandler(async (req, res) => {
  ...
  if (!paymentMethod || !gatewayOrderId) throw new ApiError(400, "paymentMethod and gatewayOrderId are required");
  if (paymentMethod !== "cod") throw new ApiError(400, "ICICI payments are verified by the ICICI return/advice flow");
  ...
  try {
    const result = await session.withTransaction(async () => {
      ...
      const verified = await gateway.verifyPayment({ order, payload: { gatewayOrderId, gatewayPaymentId, gatewaySignature } });
...
const iciciStart = asyncHandler(async (req, res) => {
  ...
  if (!order.gatewayRedirectUrl || !order.gatewayTranCtx) throw new ApiError(409, "ICICI payment session is unavailable; please restart checkout");
...
// Admin-only merchant-side UAT diagnostic wrapper. It never creates a payment.
const iciciDiagnostics = asyncHandler(async (req, res) => {
  const diagnostic = iciciProvider.getDiagnostics();
  const orderNumber = String(req.query.orderNumber || "").trim();
  if (!diagnostic.configured) return sendSuccess(res, 200, { ...diagnostic, statusCheck: null });
  if (!orderNumber) {
    return sendSuccess(res, 200, { ...diagnostic, statusCheck: { skipped: true, reason: "Pass ?orderNumber=<JATA-order-number> to run STATUS." } });
  }
  const order = await Order.findOne({ orderNumber, paymentMethod: "icici" }).select("orderNumber gatewayOrderId totalPaise currency status paymentStatus");
  if (!order) throw new ApiError(404, "ICICI order not found for diagnostic STATUS check");
  try {
    const status = await iciciProvider.checkStatus({ originalTxnNo: order.gatewayOrderId });
    return sendSuccess(res, 200, {
      ...
      statusCheck: {
        ...
        txnResponseCode: status.txnResponseCode || null,
        responseCode: status.responseCode || null,
        transactionType: status.transactionType || null,
        txnID: status.txnID || null,
        verified: true,
      },
    });
  } catch (error) {
    console.warn("[icici] diagnostic STATUS failed", { orderNumber: order.orderNumber, merchantTxnNo: order.gatewayOrderId, message: error.message });
    return sendSuccess(res, 200, { ...diagnostic, statusCheck: { skipped: false, orderNumber: order.orderNumber, merchantTxnNo: order.gatewayOrderId, verified: false, error: error.message } });
  }
});

const getByOrderNumber = asyncHandler(async (req, res) => {
  const order = await Order.findOne({ orderNumber: req.params.orderNumber }).populate("items.product", "name slug featuredImageUrl");
  ...
  const base = { orderNumber: order.orderNumber, status: order.status, paymentMethod: order.paymentMethod, paymentStatus: order.paymentStatus, items: order.items, totalPaise: order.totalPaise, currency: order.currency, createdAt: order.createdAt, paidAt: order.paidAt };
  if (!verified) return sendSuccess(res, 200, base);
  return sendSuccess(res, 200, { ...base, customerName: order.customerName, customerEmail: order.customerEmail, customerPhone: order.customerPhone, shippingAddress: order.shippingAddress, subtotalPaise: order.subtotalPaise, shippingPaise: order.shippingPaise });
});

const listMyOrders = asyncHandler(async (req, res) => {
  const orders = await Order.find({ customer: req.customer._id }).sort("-createdAt").select("orderNumber createdAt status paymentStatus paymentMethod totalPaise currency items.name items.quantity items.image paidAt collectedAt shippingAddress");
...
module.exports = { paymentMethods, iciciDiagnostics, createOrder, verifyPayment, iciciStart, iciciReturn, iciciAdvice, getByOrderNumber, listOrders, getOrderById, updateOrderStatus, updatePaymentStatus, listMyOrders };
